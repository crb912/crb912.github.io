...
具体的待办事项: [计划](http://ruibing.org/private/2099/12/28/My-Plans.html)，这篇博客与之不同。这篇侧重自己在计算机编程领域学习过程的心得体会与收获。
...
推荐了几本书，关于C++的三个方面：合法性指南，道德性指南，代码示例指南。

一些书是可选的，并不表示用这些书写得不好。比如《C++ Primer Plus》就能作为《A Tour of C++ 》的替代，并且它对没有编程基础的人更友好。

此外，对于C++编程规范，直接看ioscpp官方网页就可以了：[C++ 核心准则](http://isocpp.github.io/CppCoreGuidelines/CppCoreGuidelines) . 正如它的引言提到的那样，在不同的代码库之间实现一个更规范的代码风格。代码规范是很重要的一件事，我在这篇文章里[Importance of coding style](http://ruibing.org/programming/2099/12/26/programming.html#Importance_of_coding_style)做过解释。

### 2021.03
上个月写[C++的左值和右值](http://ruibing.org/programming/2021/02/27/cpp-lvalue-rvalue.html)，耽误了好多进度。感觉这是我写得最痛苦的一篇文章，后面不知道该如何收尾了。有时候为了把一个东西解释清楚，就不得不解释更多的东西，导致把我自己都搞晕了。


### 2021.02 
Reading *A Tour of C++*

- My "complex" arithmetic type --> [implement a simple comlex class](https://gist.github.com/crb912/bb2d94e2a5fe28b1e844d744f0b4a8cd) 
- [Concept: Resource acquisition is initialization, RAII](https://en.wikipedia.org/wiki/Resource_acquisition_is_initialization) , acquiring resources in a constructor and releasing them in a destructor.

RAII是作为资源管理的技术，它遵循的基本逻辑(两个步骤)：先请求资源(初始化)，再释放。

我感觉自己对C++移动语义的理解还不够深刻，“纸上得来终觉浅”，应该正是因为我还没有写过大量的C++代码。这篇[The deal with C++14 xvalues](https://blog.jeaye.com/2017/03/19/xvalues/) 倒数第四个代码示例，好像给出了很好的C++右值引用和使用移动语义的参考。对于C++，虽然目前我还有写不太够明白的地方，我相信随着学习C++的深入，应该一切会慢慢清晰起来。

###　一本关于计算机系统性能的书
 [William Stallings, Computer organization and architecture : designing for performance](http://home.ustc.edu.cn/~leedsong/reference_books_tools/Computer%20Organization%20and%20Architecture%2010th%20-%20William%20Stallings.pdf) 
 
做个记录，这本书好像很不错。800多页，不算薄，等有时间再看。
...
### 2020.02
- [x] long time no see! 春节期间最近打了好久的游戏, 做了会网瘾少年.把我的王者荣耀打到了最强王者段位了, 小号也打到星耀二了. 结束了! 心情有点不错, 工作和生活回到正轨.
...
- [ ]  本月目标: 我必须对自己要求更高一点, 不能只是满足于"知其然", 更要"知其所以然." 看了Python的一些标准库组件的代码实现, 我意识到我的代码还远远不够好. 一方面, 我要在工作中提升编程能力; 另一方面, 也要通过私下的自我学习, 弥补自己基础不够扎实和算法不够好的问题. 
