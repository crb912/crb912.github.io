...
- [什么是具有身份](http://ruibing.org/programming/2021/02/17/cpp-learning.html#has_indentity)
- [什么是可被移动--C++移动语义](http://ruibing.org/programming/2021/02/17/cpp-learning.html#can_be_moved_from)
...
```
当`std::move`那条语句执行之后，**注意s的值就成了空的了**，但是它的内存地址还在。就是说未定义但状态有效。

 这个表达式中的`std::move(s)`是ravlue，**更精确的说**，它是`xvalue`。它既不是lavlue，又不是prvalue。
