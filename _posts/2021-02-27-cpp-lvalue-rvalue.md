...
1. 右边的数字10是rvalue，因为它是没有内存地址的。数字10, 更精确的说，由于它是rvalue且不是xvalue，那么它是pvalue。
2. 左边的变量 `int y`是左值(lvalue)， 因为它是一个变量。

那么问题来了，在C++中，什么是变量？ 变量是命名的对象。什么是对象？对象是某特定类型的内存空间。什么是类型？类型是一组可能值及其一组操作。 不好意思，我这追根问底的毛病犯了。不过对基本概念的透彻把握，才会帮助我们理解更复杂的概念。

经过我这几个问题，你应该知道了：**变量有内存空间的，它是左值(lvalue)**. 
