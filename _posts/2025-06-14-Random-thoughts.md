...
关于性能优化，我总是会想起之前某个经历：公司的分析程序需要执行5分钟才能结束，运行的时候就要等待。我弄清楚代码之后，发现它每次都会读取一些本不应该读取的文件，造成了不必要的磁盘访问。我简单的跳过那些文件，最终程序执行的时间只需要1~2分钟。我只是修改了几行代码，就让性能提升了几倍。在我看来，性能优化更多是个伪命题，大多数项目还不足以需要考虑语言的性能。所谓代码的性能不好，往往是因为程序员对业务逻辑的理解有较大问题。

对于语言，虽然自己更擅长Python，其次是Go；但我更喜欢也更愿意专注C++。不清楚还能不能找到一份喜欢的工作，有时也需要一点运气。如果只是做简单重复的事，基本到了40岁职业生涯就结束了。在国内，程序员的工作寿命普遍很短，很少有人50岁还能继续写代码。未必因为能力不行，往往是因为没有公司要了。如果还是继续写代码，那就找到一个“厚雪长坡”的方向，最好可以持续学习，随着阅历和经验而不断提升能力的方向。只是简单重复的工作，太容易被取代了。

### 对编程工作的思考

1. 如果想写得久不被裁员，就不能考虑做网页后端。纯业务逻辑，工作容易被替代。而我特别不喜欢做网页后端。
