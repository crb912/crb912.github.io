...
hide: true
---  
## Linux启动过程的日志(syslog)一些发现（按事件先后顺序）：

- 启动过程：systemd，whoopsie会上传一些程序的崩溃日志。
- systemd-timesyncd，同步时间。
- 在tty startx之后，systemd-xdg-autostart-generator会自动加载Ubuntu的自启动脚本。
- 自启动配置在：/home/user_name/.config/autostart 目录下，桌面的配置文件中参数指明自启动。
